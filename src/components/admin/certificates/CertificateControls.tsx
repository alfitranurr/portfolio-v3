...
import { Search, X, LayoutGrid, LayoutList, Sparkles, ShieldCheck, Award, FileCheck } from 'lucide-react'
...
export function CertificateControls({
  ...
  const getCategoryIcon = (cat: string) => {
    if (cat === 'competition') return <Award className="w-3.5 h-3.5 shrink-0" />
    if (cat === 'seminar_workshop') return <FileCheck className="w-3.5 h-3.5 shrink-0" />
    if (cat === 'license_certification') return <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
    if (cat === 'committee_organization') return <Sparkles className="w-3.5 h-3.5 shrink-0" />
    return null
  }
  ...
  return (
    <div className="p-4 rounded-2xl glass-panel border border-slate-200/10 dark:border-slate-800/10 space-y-4 relative z-30">
      ...
      <div className="flex justify-center">
        <div className="flex flex-wrap p-1 rounded-2xl bg-white/5 border border-slate-200/10 dark:border-slate-800/10 max-w-4xl w-full relative gap-1">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              type="button"
      ...
      <div className="flex items-center justify-end pt-2 border-t border-slate-200/5 dark:border-slate-800/5">
        <span className="text-[11px] font-semibold text-muted-foreground shrink-0">
          Showing {displayedItems} of {totalItems} {totalItems === 1 ? 'certificate' : 'certificates'}
        </span>
      </div>
    </div>
  )
