...
export async function savePhotoAction(photoData: any) {
  ...
  if (!hasSupabaseConfig()) {
    ...
    const isEdit = !!photoData.id && (photoData.id.startsWith('mock-') || list.some((p: any) => p.id === photoData.id))
  ...
  try {
    ...
    if (error) {
      ...
      if (isTableMissing) {
        ...
        if (isEdit) {
          list = list.map((p: any) => p.id === photoData.id ? { ...p, ...dbPayload } : p)
        } else {
